...
<div class="master">
    <div class="mstxt">
        ...
        <div class="mstxt2">
            最强成就大师<img src="../assets/img/qq.png" alt="" class="msimg">atrior
        </div>
        <div class="mstxt3">
            已经达成了00项成就
        </div>
...
onMounted(()=>{
    ...
    const ms1anim = new IntersectionObserver(lists=>{
        lists.forEach(list=>{
            if(list.isIntersecting){
                list.target.classList.add('msactive')
                setTimeout(() => {
                ...
                medals.classList.remove('slide-in-top')
            }
        })
    }, {threshold: 0.5})
    const mstxt1s = document.querySelectorAll('.mstxt1, .mstxt2, .mstxt3, .mstxt4, .mstxt5, .mstxt6, .cards')
